...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($_POST['action'] === 'add_multiple') {
            ...
            $pet_uuids = json_decode($_POST['pet_uuids'] ?? '[]', true);
            ...
            if (empty($pet_uuids) || !is_array($pet_uuids)) {
                echo json_encode(['success' => false, 'message' => 'Please select at least one pet']);
                exit;
            }

            // Validate all pets belong to user
            global $conn;
            $placeholders = implode(',', array_fill(0, count($pet_uuids), '?'));
            $stmt = $conn->prepare("SELECT uuid FROM pets WHERE uuid IN ($placeholders) AND user_uuid = ?");
            $params = array_merge($pet_uuids, [$userUuid]);
            $stmt->execute($params);
            $validPets = $stmt->fetchAll(PDO::FETCH_COLUMN);

            if (count($validPets) !== count($pet_uuids)) {
            ...
            // Generate a shared booking group ID for multiple services/pets
            $bookingGroupId = uuid();
            $appointmentCount = 0;
            $successCount = 0;
            ...
            try {
                ...
                // Create appointments for each combination of pet and service
                foreach ($pet_uuids as $pet_uuid) {
                    foreach ($service_uuids as $service_uuid) {
                        $appointmentCount++;
                ...
                if ($successCount === $appointmentCount) {
                    $petCount = count($pet_uuids);
                    $serviceCount = count($service_uuids);
                    $message = $appointmentCount > 1
                        ? "Successfully booked {$appointmentCount} appointments for {$petCount} pet(s) with {$serviceCount} service(s)! We will contact you to confirm."
